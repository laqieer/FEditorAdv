...
public class SpellAnimationInserter extends Editor<SpellAnimationArray> {
	...
	public SpellAnimationInserter(View view) {
		...
		addButtonToPanel(commandSpinnerPanel, "Add Command", new ActionListener() {
			@Override
		...
		durationSpinner = new LabelledHexSpinner(
			new IntValueListener() {
				@Override
				public void newValue(int value) {}
			}, "Input frame duration:", 4,
			0, 0, 0xFFFF
		);
		durationSpinnerPanel.add(durationSpinner);
		addButtonToPanel(durationSpinnerPanel, "Insert Frame", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
				try {
		...
		addButtonToPanel(insertFramePanel, "Queue Background", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
				File loadedFile = CommonDialogs.showOpenFileDialog("image of animation frame background");
				try {
					GBAImage testConverter = processImage(loadedFile);
		...
		terminateButton = addButtonToPanel(buttonPanel, "Terminate Animation", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
		...
		arrayPanel = new ArrayPanel<SpellAnimationArray>(this, 0x1, 0xFFFF, 0);
		arrayPanel.setMaximumSize(arrayPanel.getPreferredSize());
		add(arrayPanel);

		JPanel scriptPanel = new JPanel();
		addButtonToPanel(scriptPanel, "Load From Script", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
				loadFromScript();
				update();
		...
		addButtonToPanel(scriptPanel, "Reset Animation", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
				arrayHandle.reset();
				lastPath = "N/A";
		...
		addButtonToPanel(scriptPanel, "Quit", new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent a) {
				quit();
			}
	...
	private void update() {
		...
		diagnosticLabel.setText(
			String.format(
				"0x85 command count for this spell: %d\n" +
				"Animation frame count for this spell: %d\n" +
				"Last path: %s",

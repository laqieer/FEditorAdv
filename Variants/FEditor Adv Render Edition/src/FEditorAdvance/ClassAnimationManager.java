...
public class ClassAnimationManager extends Editor<ClassAnimationArray> {
	...
	*/
	...
	public void frameDumper(File selectedFile, String frameDumpPath, String baseString) {
		try {
                    // BwdYeti: uses the rendering system instead of the logical one
			...
			for (int i = 0; i < 256; ++i) {
				if (images[i] != null) {
					ImageIO.write(
                        ...
                        Model.ScriptReader sr = new Model.ScriptReader(frameDumpPath);
						...
						try {
							...
							arrayHandle.getEntry().writeToFile(selectedFile);
                                                        if (box == true)
                                                        {
                                                            ...
                                                            frameDumper(selectedFile, parent + String.format("%s Frame Data.dmp", baseString), baseString);
                                                        }
						} catch (Exception e) {
							CommonDialogs.showCatchErrorDialog(e);
						}
				...
				new ProcessComponent(size, 1, "Dumping animation...") {
					protected boolean iterate(int i) {
						try {
                                                    ...
                                                    String animSetName = String.format("%s %02d - %s", game.baseGameName(), i, arrayHandle.getName());
                                                    ...
                                                    if (box == true)
                                                    {
                                                        frameDumper(selectedFile, folder.getPath() + File.separator + String.format("%s Frame Data.dmp", animSetName), animSetName);
                                                    }
						} catch (Exception e) {
                                                    CommonDialogs.showCatchErrorDialog(e);
						}

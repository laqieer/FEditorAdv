...
public class CGImage {
	...
	public static final int TILE_COUNT = TILE_WIDTH*TILE_HEIGHT;
	...
	private void verifyImage(BufferedImage theImage) {
		...
		if (theImage.getWidth() != WIDTH)
			throw new IllegalArgumentException(
				methodName
				+ "Input image's width isn't 240 pixels"
			);
		if (theImage.getHeight() != HEIGHT)
			throw new IllegalArgumentException(
				methodName
				+ "Input image's height isn't 160 pixels"
			);
	}
	...
	public CGImage(byte[] imageData, byte[] mapData, byte[] paletteData) {
		...
		if (
			...
			throw new IllegalArgumentException(
				methodName
				+ "mapData specifies incorrect dimensions"
			);
		if (
			...
			throw new IllegalArgumentException(
				methodName
				+ "paletteData is incorrect length"
			);

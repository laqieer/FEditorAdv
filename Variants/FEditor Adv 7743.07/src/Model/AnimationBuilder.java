...
//	public static final int MAX_OAM_DATA_LEN = 0x00005800;
	public static final int MAX_OAM_DATA_LEN = 0x00009E00;  //7743  jack-san no anime jyougenn ni awasetayo
	...
	public void addCommand(byte commandID) {
		...
		if (commandID == 0x01 && hasLoop) {
			if (wordCount > 0xff) {
				throw new RuntimeException("Loop too long!");
			}
	...
	public void addModeTerminator() throws RuntimeException {
		if (frame == 0)
			throw new RuntimeException(
				"The current mode is incomplete!"
			);
		if (mode >= MAX_MODE_COUNT)
			return;
		canLoop = false;
	...
	private GBAImage[] processFrame(GBAImage frameImage) {
		...
		if (height != 20) {
			throw new IllegalArgumentException(
				"That image has an invalid height!"
			);
		}

		if (width != 31 && !(width == 61 && include_bg)) {
			throw new IllegalArgumentException(
				"That image has an invalid width!"
			);
		}
	...
	throws RuntimeException {
		if (mode > 0 || frame > 0)
			throw new RuntimeException(
				"It's too late to specify a throwable sprite!"
			);
		if (throwableSprite == null)
			throw new RuntimeException(
				"No throwable sprite specified!"
			);
		if (
	...
	public PortableClassAnimation create() {
		for (int i = 4; i < sectionData.length; ++i)
			if (sectionData[i] == 0) {
				throw new RuntimeException("Incomplete animation!");
			}
		...
		if (frameDataCount > MAX_FRAME_DATA_LEN) {
			throw new RuntimeException(String.format("Frame data overflow! %d is over %d",frameDataCount,MAX_FRAME_DATA_LEN));
		}
		if (rightToLeft.size() * 12 > MAX_OAM_DATA_LEN) {
			throw new RuntimeException(String.format("OAM data overflow! %d is over %d",rightToLeft.size() * 12,MAX_OAM_DATA_LEN));
		}

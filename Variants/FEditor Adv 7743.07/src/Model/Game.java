...
public abstract class Game {
	...
	*/
	...
	public ROM.Pointer write(int[] data, boolean marked) {
		...
		System.out.println(
			"byteCount: 0x"
			+ Integer.toString(byteCount, 16).toUpperCase()
			+ " location: 0x"
			+ Integer.toString(location, 16).toUpperCase()
			+ " p: 0x"
			+ String.format("0x%08X", p.getLocation() + ROM.GBA_HARDWARE_OFFSET)
		);
	...
	public static Game open(String fileName) {
		...
		try {
			...
			if (id.equals(FE7JP.GAME_ID())) { result = new FE7JP(); }
			if (id.equals(FE8JP.GAME_ID())) { result = new FE8JP(); }

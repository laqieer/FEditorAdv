...
public abstract class PortraitArray extends PointerArray {
	...
	protected int getImageSize(int type) {
	...
	protected void moveToEndOfData(ROM.Pointer p, int index) {
		System.out.println("moveToEndOfData1");
		...
		System.out.println("moveToEndOfData2");
			LempelZiv.decompress(p);
		} else {
		System.out.println("moveToEndOfData3");
	...
	**/
	...
	public void setPortraitData(GBAImage input) {
		...
		System.out.println("!setPortraitData");
	...
	public int searchDataCount(int startPointer)
	{
		...
		{
			int pos =startPointer+(i*28)+ROM.GBA_HARDWARE_OFFSET;
			ROM.Pointer p = owner.getPointer(pos+4+4); //using pallet address
			int a = p.nextInt();
			if(!p.isPointer(a))
			{
				return i;

...
public class GraphicEditor extends JFrame {
	...
	private JButton selectImageButton = new JButton("选择图片");
	...
	// file)
	private JButton selectROMButton = new JButton("打开ROM");

	// Clicking this should let the user choose where to save the open ROM
	// and then save it accordingly
	private JButton saveROMButton = new JButton("保存ROM");

	// Compress image data if this is set
	private JCheckBox compressImageCheckbox = new JCheckBox("压缩图片?");

	// Compress palette data if this is set
	private JCheckBox compressPaletteCheckbox = new JCheckBox("压缩调色板?");

	// Inserts the loaded image into the open ROM (if one is open >.>)
	private JButton writeToROMButton = new JButton("写入ROM");

	// Dumps the loaded image as binary data
	private JButton writeToFileButton = new JButton("写入文件");
	...
	private void write_helper(String type, byte[] output) {
		...
		Integer address = CommonDialogs.promptForInteger(
			String.format("输入 %s 数据写入的地址:", type),
			0, size
		);
		if (address == null) { return; }

		Integer pointer = CommonDialogs.promptForInteger(
			String.format("输入要更新的 %s 指针的地址:", type),
			0, size - 4
		);
		...
		try {
			...
			CommonDialogs.showGenericErrorDialog("写入错误：检查输入!");
		}
		write = false;
	}

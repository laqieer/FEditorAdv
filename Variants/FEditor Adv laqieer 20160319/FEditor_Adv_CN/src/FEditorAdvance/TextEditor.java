...
public class TextEditor extends Editor<TextArray> {
	...
	public TextEditor(View view) {
		...
		replacePanel.add(new JLabel("替换为:"));
		...
		addButtonToPanel(panelA, "查找", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.moveTo(1); find(findField.getText(), true);
			}
		});
		addButtonToPanel(panelA, "查找下一个", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.next(); find(findField.getText(), true);
			}
		});
		addButtonToPanel(panelA, "查找前一个", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.prev(); find(findField.getText(), false);
			}
		});
		addButtonToPanel(panelA, "全部替换", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				replaceAll(findField.getText(), replaceField.getText());
			}
		});
		regExChkBox = new JCheckBox("使用Reg Ex?");
		...
		addButtonToPanel(panelB, "应用", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
		 		applyChanges();
		 	}
		 });
		 addButtonToPanel(panelB, "回滚", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
		...
		addButtonToPanel(panelB, "插入...", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				insert();
			}
		});
		addButtonToPanel(panelB, "退出", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				quit();
			}
		});
	...
	**/
	...
	private void find(final String toFind, final boolean forward) {
		...
		final Finder f = new Finder(
			arrayHandle.getCurrentSize(), 1, "文本查找中..."
		);
		...
		view.process(
			...
			new PositionRestoringProcess(
				new ProcessComponent(
					arrayHandle.getCurrentSize(), 1, "文本出现替换中..."
				) {
					@Override
					protected boolean iterate(int i) {
						if (i == 0) { return false; }
	...
	private void dump() {
		...
		File selectedFile = CommonDialogs.showSaveFileDialog("Dump脚本");
		...
		view.process(
			true, // show dialogs
			new PositionRestoringProcess(
				new ProcessComponent(size, 1, "脚本Dump中...") {
					@Override
					protected boolean iterate(int i) {
						if (i == 0) { return false; }
						arrayHandle.moveTo(i);
	...
	private void insert() {
		...
		File fileSelected = CommonDialogs.showOpenFileDialog("Dump脚本");

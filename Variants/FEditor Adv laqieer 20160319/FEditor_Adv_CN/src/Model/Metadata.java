...
public class Metadata {
	...
	private void initSpellArrayAddress(ROM rom, byte[] data) {
		if (data.length != 4) { throw new RuntimeException("无效部分!"); }
	...
	private int writeChunk(
		...
		if ((size % 4) != 0) { throw new RuntimeException("损坏部分!"); }
		stream.write(chunk);

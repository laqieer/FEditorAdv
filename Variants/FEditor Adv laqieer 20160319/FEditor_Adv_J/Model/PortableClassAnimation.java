...
public class PortableClassAnimation implements Serializable {
		...
		for (int command: Util.bytesToInts(frameData)) {
			switch (mode) {
				...
				if (frame == requestedFrame) {
					return renderSingleRTL(sheet, command);
				}
		...
		for (int command: Util.bytesToInts(frameData)) {
			switch (mode) {
				...
				if (result[frame] == null) {
					result[frame] = renderSingleRTL(sheet, command);
				}
				mode = 0;
				break;
			}
	...
	public BufferedImage renderSingleRTL(byte[] sheetData, int offset) {
		...
		BufferedImage result = new GBAImage(p, 30, 20).getImage();
		...
		renderOAM(source, destination, offset);

		return result;
	}

	private BufferedImage renderSingleRTL(int sheet, int offset) {
		return renderSingleRTL(sheets.get(sheet), offset);
	}

	private void renderOAM(
		BufferedImage source, Graphics2D destination, int offset
	) {
		if (offset == rightToLeftOAM.length) return;
		OAM oam = OAM.deserialize(rightToLeftOAM, offset, false);
		if (oam == null) return;

		renderOAM(source, destination, offset + 12);
	...
	public PortableClassAnimation(
		Game source,
		int[] name, ROM.Pointer rawSectionData, ROM.Pointer rawFrameData,

...
public class ClassAnimationManager extends Editor<ClassAnimationArray> {
	...
	public ClassAnimationManager(View view) {
		...
		addButtonToPanel(clipboardPanel, "クリップボードにDump", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				dumpToClipboard();
			}
		});
		addButtonToPanel(clipboardPanel, "クリップボードから貼り付け", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				pasteFromClipboard();
			}
		});
		addButtonToPanel(clipboardPanel, "クリップボードをクリア", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				clearClipboard();
			}
		});
		...
		addButtonToPanel(operationPanel, "名前", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				setName();
			}
		});
		addButtonToPanel(operationPanel, "保存...", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				dump();
			}
		});
		addButtonToPanel(operationPanel, "挿入...", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				insert();
			}
		});
		addButtonToPanel(operationPanel, "終了", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				quit();
			}
		});
	...
	*/
	...
	public void dump() {
		final File selectedFile = CommonDialogs.showSaveFileDialog("アニメーションDump...");
		if (selectedFile == null) { return; }

		view.process(
			false, // Don't show dialogs
			new PositionRestoringProcess(
				new ProcessComponent(1, 1, "アニメーションのDump中...") {
	...
	public void insert() {
		final File selectedFile = CommonDialogs.showOpenFileDialog("アニメーションDump...");
		if (selectedFile == null) { return; }

		view.process(
			false, // Don't show dialogs
			new PositionRestoringProcess(
				new ProcessComponent(1, 1, "アニメーションを挿入しています...") {
					protected boolean iterate(int i) {
						try {

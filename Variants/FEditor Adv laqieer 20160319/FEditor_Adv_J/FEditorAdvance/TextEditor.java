...
public class TextEditor extends Editor<TextArray> {
	...
	public TextEditor(View view) {
		...
		findPanel.add(new JLabel("テキストを検索する:"));
		...
		replacePanel.add(new JLabel("置換するテキストを入力:"));
		...
		addButtonToPanel(panelA, "検索", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.moveTo(1); find(findField.getText(), true);
			}
		});
		addButtonToPanel(panelA, "次を検索", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.next(); find(findField.getText(), true);
			}
		});
		addButtonToPanel(panelA, "前を検索", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				arrayHandle.prev(); find(findField.getText(), false);
			}
		});
		addButtonToPanel(panelA, "すべて置き換え", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				replaceAll(findField.getText(), replaceField.getText());
			}
		});
		regExChkBox = new JCheckBox("レッグ例を使用しますか?");
		...
		addButtonToPanel(panelB, "適用する", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
		 		applyChanges();
		 	}
		 });
		 addButtonToPanel(panelB, "戻す", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
		...
		addButtonToPanel(panelB, "挿入...", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				insert();
			}
		});
		addButtonToPanel(panelB, "終了", new ActionListener() {
			public void actionPerformed(ActionEvent a) {
				quit();
			}
		});
	...
	**/
	...
	private void find(final String toFind, final boolean forward) {
		...
		final Finder f = new Finder(
			arrayHandle.getCurrentSize(), 1, "テキストのサーチ中..."
		);
		...
		view.process(
			...
			new PositionRestoringProcess(
				new ProcessComponent(
					arrayHandle.getCurrentSize(), 1, "テキスト置き換えの出現..."
				) {
					@Override
					protected boolean iterate(int i) {
						if (i == 0) { return false; }
	...
	private void dump() {
		...
		File selectedFile = CommonDialogs.showSaveFileDialog("スクリプトDump");
		...
		view.process(
			true, // show dialogs
			new PositionRestoringProcess(
				new ProcessComponent(size, 1, "スクリプトのDump中...") {
					@Override
					protected boolean iterate(int i) {
						if (i == 0) { return false; }
						arrayHandle.moveTo(i);
	...
	private void insert() {
		...
		File fileSelected = CommonDialogs.showOpenFileDialog("スクリプトのDump");
		...
		view.process(
			...
			new PositionRestoringProcess(
				...
				new ProcessComponent(1, 0, "スクリプトの読み込み...") {
					@Override
					protected boolean iterate(int i) {
						int index = 1;

 ...
 *  (max index = size - 1) of a pointer table.
 */
...
public class TablePanel<T extends PointerArray> extends JPanel {
	...
	public TablePanel(Editor<T> editor, int first_edit_index, int max_edit_index, int offset) {
		this.editor = editor;
	...
	private void refresh(int value) {
		// In some cases, the table will already be at this position, but it's
		// cheaper and simpler to set the position blindly than to check.
		table.moveTo(value - offset);
		editor.refresh();
	}
